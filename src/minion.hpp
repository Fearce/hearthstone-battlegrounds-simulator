...
struct Minion
{
  ...
  constexpr Minion()
      : attack(0), health(0), type(MinionType::None), golden(false), taunt(false), divine_shield(false), poison(false), windfury(false), reborn(false), deathrattle_murlocs(0), deathrattle_microbots(0), deathrattle_golden_microbots(0), deathrattle_plants(0), attack_aura(0), health_aura(0), invalid_aura(false)
  {
  }
...
private:
  inline constexpr Minion(MinionType type, MinionInfo const &info, bool golden)
      : attack(info.attack_for(golden)), health(info.health_for(golden)), type(type), golden(golden), taunt(info.taunt), divine_shield(info.divine_shield), poison(info.poison), windfury(info.windfury), reborn(false), deathrattle_murlocs(0), deathrattle_microbots(0), deathrattle_golden_microbots(0), deathrattle_plants(0), attack_aura(0), health_aura(0), invalid_aura(false)
  {
  }
...
public:
  ...
  void on_attack(Battle &battle, int player, int pos, int attack);
  ...
  void on_battle_start(Battle& battle, int player, Board &board);
  ...
  constexpr Minion(MinionType type, bool golden, int attack, int health, bool taunt, bool divine_shield, bool poison, bool windfury, bool reborn, int deathrattle_murlocs, int deathrattle_microbots, int deathrattle_golden_microbots, int deathrattle_plants)
      : attack(attack), health(health), type(type), golden(golden), taunt(taunt), divine_shield(divine_shield), poison(poison), windfury(windfury), reborn(reborn), deathrattle_murlocs(deathrattle_murlocs), deathrattle_microbots(deathrattle_microbots), deathrattle_golden_microbots(deathrattle_golden_microbots), deathrattle_plants(deathrattle_plants), attack_aura(0), health_aura(0), invalid_aura(false)
  {
  }
};
...
inline ostream &operator<<(ostream &s, Minion const &minion)
{
  ...
  if (minion.reborn)
    s << ", reborn";
  for (int i = 0; i < minion.deathrattle_microbots; i += 3)
    s << ", microbots";
  for (int i = 0; i < minion.deathrattle_golden_microbots; i += 3)
    s << ", golden microbots";
  for (int i = 0; i < minion.deathrattle_plants; i += 2)
    s << ", plants";
  return s;
}
